...
# Pandora helper functions

def pad(s, l):
    return s + "\0" * (l - len(s))

def pandora_encrypt(key, s):
    blowfish_encode = Blowfish(key)
    return "".join([blowfish_encode.encrypt(pad(s[i:i+8], 8)).encode('hex') for i in xrange(0, len(s), 8)])

def pandora_decrypt(key, s):
    blowfish_decode = Blowfish(key)
